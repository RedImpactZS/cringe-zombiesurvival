...
function ENT:Draw()
	...
	cam.Start3D2D(self:LocalToWorld(Vector(1, 0, self:OBBMaxs().z)), self:GetAngles(), 0.05)
		...
		draw.SimpleText(translate.Get("bucket"), "ZS3D2DFont2", -20, -100, COLOR_GRAY, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		...
		if MySelf:Team() == TEAM_HUMAN and GAMEMODE:PlayerCanPurchase(MySelf) then
			...
			draw.SimpleText(translate.Get("fuck youu"), "ZS3D2DFont2", -15, 0, colFlash, TEXT_ALIGN_CENTER)
		end
